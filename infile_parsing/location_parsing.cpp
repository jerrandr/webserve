...
static int      location_check(std::string line, Location &locs)
{
    ...
    if (it != locs_keys.end())
    {
        insert_value(key_locs, line_value, locs);
        return (1);
    }
    (void) locs;
...
static void     other_uri_value(std::string line, Location &locs)
{
    size_t     pos;
    int        len;
    std::string one_line;
...
int     get_principal_uri(std::string value, Config &cfg)
{
    ...
    other_uri_value(last_line, locs);
    cfg.set_locs(locs);
    return (1);
};